 ...
 * Starter templates are seeded on first install and can be deleted by users.
 */
...
function _orderTemplates(templates) {
...
async function getTemplates() {
  ...
  return _orderTemplates(data[STORAGE_KEYS.templates] || []);
}
...
async function saveTemplate(template) {
  ...
  if (template.id) {
    ...
    if (idx !== -1) {
      ...
      await chrome.storage.sync.set({ [STORAGE_KEYS.templates]: _orderTemplates(templates) });
...
async function deleteTemplate(id) {
  ...
  await chrome.storage.sync.set({ [STORAGE_KEYS.templates]: _orderTemplates(filtered) });

  // Fall back default if deleted template was default
  const defaultId = await _getDefaultId();
  if (defaultId === id) {
    await chrome.storage.sync.set({
      [STORAGE_KEYS.defaultId]: filtered[0] ? filtered[0].id : null,
    });
  }
}
